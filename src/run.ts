...
export default function run({
  ...
  // ID containing date and time plus random string
    ...
    for (const step of steps) {
      ...
      switch (step.type) {
        ...
        case "extract_embeddings": {
          ...
          const method =
            step.with.input_type == "pdf"
              ? extractEmbeddingsFromPdf
              : extractEmbeddingsFromWebpage;
        ...
        case "save_to_vector_db": {
          ...
          const notes = replacePlaceholders(step.with.notes);
  ...
  async function extractEmbeddingsFromWebpage(
    webpageUrl: string,
    wordsPerChunk?: number,
    chunkOverlapPercent?: number,
    outputKey?: string,
    ...
    const data: string[] = await fetchWebpage(webpageUrl).then((pdfData) =>
      extractTextChunksFromWebpage(
        pdfData,
        chunkSize,
        chunkOverlapPercent as number
      )
  ...
  async function saveToVectorDatabase(
    ...
    for (const document of documents) {
      ...
      const { data, error } = await supabaseClient
        .from(tableName)
        .insert({
          ingest_identifier: ingestIdentifier,
          content: document,
          embedding,
          metadata,
  ...
  async function extractTextChunksFromPdf(
    pdfData: any,
    chunkSize: number,
    chunkOverlapPercent: number
  ): Promise<string[]> {
  ...
  // Function to extract text from the PDF and break it into chunks
  async function extractTextChunksFromWebpage(
    html: any,
    chunkSize: number,
    chunkOverlapPercent: number
  ): Promise<string[]> {
    ...
    // Break the text into chunks of desired size
    for (let i = 0; i < words.length; i += chunkIncrement) {
